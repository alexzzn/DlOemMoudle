...
{
    NSLog(@"%@", editorController.bodyText);
}
...
//    NSLog(@"Editor stat tracked: %lu", (unsigned long)stat);
}
...
- (void)addImageDataToContent:(NSData *)imageData
{
    ...
    NSString *domainPath = [NSSearchPathForDirectoriesInDomains(NSDocumentDirectory, NSUserDomainMask, YES).firstObject stringByAppendingPathComponent:@"img"];
    NSString *path = [NSString stringWithFormat:@"%@/%@.jpg", domainPath, imageID];
    NSError *err = nil;


    if (![[NSFileManager defaultManager] fileExistsAtPath:domainPath]) {
        [[NSFileManager defaultManager] createDirectoryAtPath:domainPath withIntermediateDirectories:YES attributes:nil error:nil];
    };

    [imageData writeToFile:path options:NSDataWritingFileProtectionComplete error:&err];
    NSLog(@"%@",err);
